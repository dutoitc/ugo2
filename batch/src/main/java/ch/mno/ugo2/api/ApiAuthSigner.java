...
public class ApiAuthSigner {
    ...
    private final String secret; // raw secret

    public void sign(HttpHeaders headers, String method, String path, byte[] body) {
        ...
        String toSign = keyId + "\n" + ts + "\n" + nonce + "\n" + method.toUpperCase() + "\n" + path + "\n" + bodyHash;
        String sig = hmacBase64(secret, toSign);
        headers.set("X-API-KEY", keyId);
        headers.set("X-API-TS", String.valueOf(ts));
        headers.set("X-API-NONCE", nonce);
        headers.set("X-API-SIG", sig);
